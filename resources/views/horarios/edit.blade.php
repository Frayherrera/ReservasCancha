...
@push('styles')
    ...
    <style>
        ...
        .editar-horario-container::before {
            content: '';
            position: fixed;
            inset: 0;
            background:
...
<div class="editar-horario-container">
    ...
    <div class="form-card">
        <h2 class="form-title">
            <i class='bx bx-edit'></i> Editar Horario
        </h2>
        <p class="form-subtitle">Modifica los datos del horario seleccionado</p>

        <form action="{{ route('horarios.update', $horario) }}" method="POST" id="horariosForm">
            @csrf
            @method('PUT')
...
function updatePreview() {
    ...
    if (fecha && hora) {
        ...
        previewDate.textContent = fechaFormateada;
        previewTime.textContent = hora + ' - ' + horaFin;
        previewStatus.textContent = estado;
        previewStatus.className = 'preview-status ' + estado.toLowerCase().replace(' ', '.');
...
document.getElementById('horariosForm').addEventListener('submit', function(event) {
    ...
    Swal.fire({
        title: '¿Estás seguro?',
        text: '¿Deseas guardar los cambios en este horario?',
        icon: 'warning',
